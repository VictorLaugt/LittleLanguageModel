...
class CharRNN(nn.Module, ReccurentLanguageModelInterface):
    ...
    def initial_hidden_states(self, device, batch_size=None):
        ...
        if batch_size is None:
            hidden_states = torch.zeros(self.rnn.num_layers, self.rnn.hidden_size)
        else:
            hidden_states = torch.zeros(self.rnn.num_layers, batch_size, self.rnn.hidden_size)
    ...
    def forward(self, input_seq, initial_hidden_states):
        ...
        outputs, hidden_states = self.rnn(embedded_tokens, initial_hidden_states)
